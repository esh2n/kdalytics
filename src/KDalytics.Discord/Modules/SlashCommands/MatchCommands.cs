...
public class MatchCommands : InteractionModuleBase<SocketInteractionContext>
{
    ...
    public MatchCommands(PlayerApiClient playerApi, MatchApiClient matchApi, PerformanceApiClient performanceApi)
    {
    ...
    [SlashCommand("recent", "プレイヤーの最近の試合を表示します")]
    public async Task GetRecentMatchesAsync(
        [Summary("name", "プレイヤー名")] string name,
        [Summary("tag", "プレイヤータグ（#以降）")] string tag,
        [Summary("count", "表示する試合数（最大5）")] int count = 3)
    {
        await DeferAsync();
        ...
        try
        {
            ...
            {
                await FollowupAsync($"プレイヤー '{player.DisplayName}' の最近の試合が見つかりませんでした。");
                return;
            }
    ...
    [SlashCommand("detail", "試合の詳細情報を表示します")]
    public async Task GetMatchDetailAsync(
        [Summary("match_id", "試合ID")] string matchId)
    {
        await DeferAsync();
        ...
        try
        {
            ...
            if (match == null)
            {
                await FollowupAsync($"試合ID '{matchId}' が見つかりませんでした。");
                return;
            }
    ...
    [SlashCommand("count", "プレイヤーの日別試合数を表示します")]
    public async Task GetMatchCountByDayAsync(
        [Summary("name", "プレイヤー名")] string name,
        [Summary("tag", "プレイヤータグ（#以降）")] string tag,
        [Summary("days", "表示する日数（最大30）")] int days = 7)
    {
        await DeferAsync();
        ...
        try
        {
            ...
            {
                await FollowupAsync($"プレイヤー '{player.DisplayName}' の試合数情報が見つかりませんでした。");
                return;
            }
